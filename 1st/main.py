...
class MainWindow(tk.Tk):
    def __init__(self, parent):
        ...
        self.lbl_result = tk.Label(self, textvariable=tk.StringVar(
            value=self.selected_group), font=("Arial", 11))
        self.lbl_result.pack(pady=5)

        menubar = tk.Menu(self)
        menubar.add_command(label="Робота1", command=self.run_work1)
        menubar.add_command(label="Робота2", command=self.run_work2)
        ...
        def popa1(self):
            ...
            while (step > 0):
                ...
                if step == 2:
                    ...
                    if res == 1:
                        messagebox.showinfo("Some info IDK")
                        break
                    elif res == -1:
                        step = 1
                        break
